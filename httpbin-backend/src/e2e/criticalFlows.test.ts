...
jest.mock(
  '../infrastructure/database/repositories/MongoHttpbinResponseRepository',
);
...
describe('End-to-End Tests for Critical User Flows', () => {
  ...
  beforeAll((done) => {
    ...
    repository =
      new MongoHttpbinResponseRepository() as jest.Mocked<MongoHttpbinResponseRepository>;
    const wsService = new SocketIOService(io);
    const monitor = new MonitorHttpbin(repository, wsService);
    controller = new HttpbinController(
      repository,
    ) as jest.Mocked<HttpbinController>;
