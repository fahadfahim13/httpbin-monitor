...
export class HttpbinController {
  ...
  async getAll(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      logger.info('Successfully retrieved all responses', {
  ...
  async getByDateRange(req: Request, res: Response): Promise<void> {
    ...
    logger.info('Received request to get responses by date range', {
      startDate,
      endDate,
    });
    try {
      ...
      logger.info('Successfully retrieved responses by date range', {
        count: responses.length,
      });
      res.json(responses);
    } catch (error) {
